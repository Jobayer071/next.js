...
describe('required server files app router', () => {
  ...
  it('should properly handle a request by an html-limited bot', async () => {
    let res = await fetchViaHTTP(appPort, '/isr/first', undefined, {
      headers: {
        // Here we're using a DuckDuckBot user agent to simulate a request by an
        // html-limited bot.
        'user-agent':
          'DuckDuckBot/1.1; (+http://duckduckgo.com/duckduckbot.html)',
        'x-matched-path': '/isr/first',
        // We're excluding the x-now-route-matches header here because we're
        // simulating the case where a bot request bypasses the prerender and
        // directly hits the handler, where it should be performing a full SSR
    ...
    // We expect that because this is a html-limited bot request, we should
    // be returning a full SSR response, which does not use React streaming.
    expect(html).toContain('<html') // Opening HTML tag
    expect(html).toContain('/html>') // Closing HTML tag
    expect(html).not.toContain('<!-- PPR_BOUNDARY_SENTINEL -->') // PPR boundary sentinel

    const $ = cheerio.load(html)

    // We expect that it should render the correct page, and it should have the
    // parameters available in the page.
    expect($('#page').text()).toBe('/isr/[slug]')
    expect($('#params').text()).toBe('{"slug":"first"}')

    res = await fetchViaHTTP(appPort, '/isr/first.rsc', undefined, {
      headers: {
        'user-agent':
          'DuckDuckBot/1.1; (+http://duckduckgo.com/duckduckbot.html)',
        'x-matched-path': '/isr/first',
  ...
  // We're skipping this test right now because this test suite has been adapted
  // for RDC for Navigations, which has different behavior for Dynamic RSC
  // requests.
  it.skip('should handle RSC requests', async () => {
